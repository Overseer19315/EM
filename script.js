...
function save(key, value) {
    try {
        ...
        console.error("DIVI-64 STORAGE ERROR:", error);
    }
}
...
/* =========================================================
   KANE SETTINGS
   NORMALIZED TO PREVENT OLD LOCALSTORAGE DATA
   FROM BREAKING INITIALIZATION
========================================================= */

let kaneSettings = load(
    STORAGE.KANE_SETTINGS,
    {
        mode: "EXECUTIVE",
        silent: false,
        context: []
...
if (
    !kaneSettings ||
    typeof kaneSettings !== "object" ||
    Array.isArray(kaneSettings)
) {
    kaneSettings = {
        mode: "EXECUTIVE",
        silent: false,
...
if (
    kaneSettings.mode !== "EXECUTIVE" &&
    kaneSettings.mode !== "ANALYST"
) {
    kaneSettings.mode = "EXECUTIVE";
}

if (typeof kaneSettings.silent !== "boolean") {
...
let scenarioHistory = load(
    STORAGE.SCENARIOS,
    []
);

if (!Array.isArray(scenarioHistory)) {
    scenarioHistory = [];
...
let sessionStats = load(
    STORAGE.SESSION_STATS,
    {
        failedLogins: 0,
        securityEvents: 0,
        activeSessions: 0
...
if (
    typeof sessionStats.failedLogins !== "number"
) {
    sessionStats.failedLogins = 0;
}

if (
    typeof sessionStats.securityEvents !== "number"
) {
    sessionStats.securityEvents = 0;
}
...
function addKaneMessage(
    ...
    row.innerHTML = `
        <div class="kaneMessageRole">
            ${escapeHTML(role)}
        </div>

        <div class="kaneMessageText">
            ${escapeHTML(message)}
        </div>
    `;
...
function addKaneContext(
    ...
    if (
        kaneSettings.context.length > 30
    ) {
...
function isKaneLocked(command) {
    ...
    return kaneState.lockedFunctions
        .includes(command);
}
...
function lockKaneFunction(command) {
    ...
    if (
        !kaneState.lockedFunctions.includes(
            command
        )
    ) {

        kaneState.lockedFunctions.push(
            command
        );
    }
...
function openKillswitch() {
    ...
    html(
        ...
        <div style="margin-bottom:15px">
            ACTIVE EVENT:
            ${escapeHTML(kaneState.currentEvent.level)}
        </div>
    ...
    document
        .querySelectorAll(".killswitchOption")
        .forEach(button => {
...
function executeKillswitch(code) {
    ...
    */
    ...
    if (
        ...
        !allowed[currentUser.id].includes(code)
    ) {

        showKaneAlert(
            "ACTION DENIED\n\nINSUFFICIENT KILLSWITCH AUTHORITY."
        );
...
function startScenario() {
    ...
    document
        .querySelectorAll(".scenarioOption")
        .forEach(button => {
            ...
            .map(person => `
                <tr>
                    ...
                    <td>
                        ...
                        <button
                            onclick="personnelAction('${escapeHTML(person[0])}')"
                            style="
                                background:#090909;
                                border:1px solid #333;
                                color:#777;
...
function init() {
    ...
    try {
        ...
        /*
           Initialization errors are logged but
           MUST NOT prevent the boot sequence.
        */

        console.error(
            "DIVI-64 INITIALIZATION ERROR:",
            error
        );
...
/* =========================================================
   STARTUP
   BOOT STARTS FIRST SO A KANE/LOCALSTORAGE ERROR
   CAN NEVER FREEZE THE LOADING SCREEN.
========================================================= */
